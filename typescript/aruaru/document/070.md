# #070 「`ts-node --esm`と格闘する深夜」制作ドキュメント

## 台本
ずんだもん「TypeScript書いてるとさ、`ts-node --esm`と格闘する深夜って絶対あるよね？」
四国めたん「わかります。ESMとCJSの狭間で迷子になるのは週間行事ですよね」
ずんだもん「しかもそのまま`ts-node --esm`と格闘する深夜状態になって、チーム全員が固まるんだよ！」
四国めたん「そこでTypeScriptのモジュール解像度の整理を使えば、`module`と`moduleResolution`を合わせて揃えるのが近道です」
ずんだもん「いや待って、それやると`ts-node --esm`と格闘する深夜第2ラウンドが始まるやつ！」
...
## 制作メモ
- **想定シーン**: `ts-node --esm`と格闘する深夜に遭遇した瞬間を切り取るネタ
- **狙いトーン**: 混乱を笑いに変えるカオス系
