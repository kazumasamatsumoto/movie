# #099 「`type predicate`を書いたのに`boolean`扱いされる」制作ドキュメント

## 台本
ずんだもん「TypeScript書いてるとさ、`type predicate`を書いたのに`boolean`扱いされるって絶対あるよね？」
四国めたん「わかります。`unknown`を`any`に戻したくなる誘惑と戦い続けますよね」
ずんだもん「しかもそのまま`type predicate`を書いたのに`boolean`扱いされる状態になって、チーム全員が固まるんだよ！」
四国めたん「そこでTypeScriptの`unknown`ガードを使えば、型ガードで`unknown`を段階的に狭めれば安心です」
ずんだもん「いや待って、それやると`type predicate`を書いたのに`boolean`扱いされる第2ラウンドが始まるやつ！」
...
## 制作メモ
- **想定シーン**: `type predicate`を書いたのに`boolean`扱いされるに遭遇した瞬間を切り取るネタ
