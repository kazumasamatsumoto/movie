# #092 「`PromiseLike`ってなんで存在するんだ問題」制作ドキュメント

## 台本
ずんだもん「TypeScript書いてるとさ、`PromiseLike`ってなんで存在するんだ問題って絶対あるよね？」
四国めたん「わかります。Promiseチェーンを追ってると朝を迎えますよね」
ずんだもん「しかもそのまま`PromiseLike`ってなんで存在するんだ問題状態になって、チーム全員が固まるんだよ！」
四国めたん「そこでTypeScriptの`Awaited`と非同期型整備を使えば、`Awaited`で戻り値を先に取り出しておくと安心です」
ずんだもん「いや待って、それやると`PromiseLike`ってなんで存在するんだ問題第2ラウンドが始まるやつ！」
...
## 制作メモ
- **想定シーン**: `PromiseLike`ってなんで存在するんだ問題に遭遇した瞬間を切り取るネタ
- **狙いトーン**: 深夜デバッグ感を演出
