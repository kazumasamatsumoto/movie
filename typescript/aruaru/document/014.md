# #014 「`strictNullChecks`をONにしたらプロジェクトが燃えた」制作ドキュメント

## 台本
ずんだもん「TypeScript書いてるとさ、`strictNullChecks`をONにしたらプロジェクトが燃えたって絶対あるよね？」
四国めたん「わかります。`unknown`を`any`に戻したくなる誘惑と戦い続けますよね」
ずんだもん「しかもそのまま`strictNullChecks`をONにしたらプロジェクトが燃えた状態になって、チーム全員が固まるんだよ！」
四国めたん「そこでTypeScriptの`unknown`ガードを使えば、型ガードで`unknown`を段階的に狭めれば安心です」
ずんだもん「いや待って、それやると`strictNullChecks`をONにしたらプロジェクトが燃えた第2ラウンドが始まるやつ！」
...
## 制作メモ
- **想定シーン**: `strictNullChecks`をONにしたらプロジェクトが燃えたに遭遇した瞬間を切り取るネタ
