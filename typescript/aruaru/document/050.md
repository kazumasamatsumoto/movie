# #050 「`emitDecoratorMetadata`が必要か論争」制作ドキュメント

## 台本
ずんだもん「TypeScript書いてるとさ、`emitDecoratorMetadata`が必要か論争って絶対あるよね？」
四国めたん「わかります。デコレータONにした瞬間ビルドが怒りますよね」
ずんだもん「しかもそのまま`emitDecoratorMetadata`が必要か論争状態になって、チーム全員が固まるんだよ！」
四国めたん「そこでTypeScriptのデコレータ設定を使えば、`experimentalDecorators`と`emitDecoratorMetadata`を揃えましょう」
ずんだもん「いや待って、それやると`emitDecoratorMetadata`が必要か論争第2ラウンドが始まるやつ！」
...
## 制作メモ
- **想定シーン**: `emitDecoratorMetadata`が必要か論争に遭遇した瞬間を切り取るネタ
- **狙いトーン**: 設定と実装の二刀流あるある
