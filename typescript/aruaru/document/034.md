# #034 「`Generics`にGenericsを重ねて悟りを開く瞬間」制作ドキュメント

## 台本
ずんだもん「TypeScript書いてるとさ、`Generics`にGenericsを重ねて悟りを開く瞬間って絶対あるよね？」
四国めたん「わかります。ジェネリクスを重ねると誰も読めなくなるんですよね」
ずんだもん「しかもそのまま`Generics`にGenericsを重ねて悟りを開く瞬間状態になって、チーム全員が固まるんだよ！」
四国めたん「そこでTypeScriptのジェネリクス設計を使えば、`extends`やデフォルト型で読みやすさを保てます」
ずんだもん「いや待って、それやると`Generics`にGenericsを重ねて悟りを開く瞬間第2ラウンドが始まるやつ！」
...
## 制作メモ
- **想定シーン**: `Generics`にGenericsを重ねて悟りを開く瞬間に遭遇した瞬間を切り取るネタ
- **狙いトーン**: 数学っぽさを軽く茶化す
