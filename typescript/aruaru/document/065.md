# #065 「`noUncheckedIndexedAccess`にしばかれる配列操作」制作ドキュメント

## 台本
ずんだもん「TypeScript書いてるとさ、`noUncheckedIndexedAccess`にしばかれる配列操作って絶対あるよね？」
四国めたん「わかります。`unknown`を`any`に戻したくなる誘惑と戦い続けますよね」
ずんだもん「しかもそのまま`noUncheckedIndexedAccess`にしばかれる配列操作状態になって、チーム全員が固まるんだよ！」
四国めたん「そこでTypeScriptの`unknown`ガードを使えば、型ガードで`unknown`を段階的に狭めれば安心です」
ずんだもん「いや待って、それやると`noUncheckedIndexedAccess`にしばかれる配列操作第2ラウンドが始まるやつ！」
...
## 制作メモ
- **想定シーン**: `noUncheckedIndexedAccess`にしばかれる配列操作に遭遇した瞬間を切り取るネタ
