# #097 「`Conditional Type`の優先順位で頭抱える」制作ドキュメント

## 台本
ずんだもん「TypeScript書いてるとさ、`Conditional Type`の優先順位で頭抱えるって絶対あるよね？」
四国めたん「わかります。ジェネリクスを重ねると誰も読めなくなるんですよね」
ずんだもん「しかもそのまま`Conditional Type`の優先順位で頭抱える状態になって、チーム全員が固まるんだよ！」
四国めたん「そこでTypeScriptのジェネリクス設計を使えば、`extends`やデフォルト型で読みやすさを保てます」
ずんだもん「いや待って、それやると`Conditional Type`の優先順位で頭抱える第2ラウンドが始まるやつ！」
...
## 制作メモ
- **想定シーン**: `Conditional Type`の優先順位で頭抱えるに遭遇した瞬間を切り取るネタ
- **狙いトーン**: 数学っぽさを軽く茶化す
