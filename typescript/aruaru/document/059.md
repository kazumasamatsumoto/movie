# #059 「`@types`更新で突然型が変わる恐怖」制作ドキュメント

## 台本
ずんだもん「TypeScript書いてるとさ、`@types`更新で突然型が変わる恐怖って絶対あるよね？」
四国めたん「わかります。型定義の配布で`types`指定を忘れて炎上したことありますよね」
ずんだもん「しかもそのまま`@types`更新で突然型が変わる恐怖状態になって、チーム全員が固まるんだよ！」
四国めたん「そこでTypeScriptの型配布フローを使えば、`types`とバンドラのdts出力を合わせれば安全に公開できます」
ずんだもん「いや待って、それやると`@types`更新で突然型が変わる恐怖第2ラウンドが始まるやつ！」
...
## 制作メモ
- **想定シーン**: `@types`更新で突然型が変わる恐怖に遭遇した瞬間を切り取るネタ
